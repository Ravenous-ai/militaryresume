...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create militaryresume-60da
  fly apps create militaryresume-60da-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app militaryresume-60da
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app militaryresume-60da-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'militaryresume-60da-staging' does not match app name in config file 'militaryresume-60da'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name militaryresume-60da-db
  fly postgres attach --app militaryresume-60da militaryresume-60da-db

  fly postgres create --name militaryresume-60da-staging-db
  fly postgres attach --app militaryresume-60da-staging militaryresume-60da-staging-db
  ```
